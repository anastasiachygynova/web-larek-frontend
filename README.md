...
**Назначение:** отображение главной страницы интернет-магазина с корзиной и каталогом товаров и счетчиком корзины
**Поля класса**
protected CartCounter - счетчик товара в корзине
protected basketButton - кнопка корзины
protected catalogContainer - контейнер галереи товаров
**Конструктор:**
constructor(protected container: HTMLElement, events: IEvents) - передаёт контейнер и объект событий в конструктор родительского класса 
**Методы:**

- `set catalog()`  - отображает массив карточек
- `setCartCounter()` - обновляет счётчик товаров в корзине
...
### MainPagePresenter
**Назначение:**  управляет логикой главной страницы, связывает данные (Model) и отображение (View)
**Конструктор:**
в конструктор передаются экземпляры классов View (MainPageView), Model (BasketModel, ApiModel) и объект событий (IEvents).
  **Методы:** 
  - `init()` - загрузка данных, первичная отрисовка, подписка на события
  - `createProductElement()` - cоздание DOM-элемента карточки товара 
